...
/* メモ再帰 */
int dp[(1<<20) + 1][20]; // dpテーブルは余裕をもったサイズにする
...
int main(){
    ...
    cin >> N;
    for (int i=0;i<N;i++){
        cin >> alist[i][0] >> alist[i][1] >> alist[i][2];
    }
    for (int i = 0; i < N; ++i) {
        for (int j = 0; j < N; ++j) {
            dist[i][j] = abs(alist[j][0]-alist[i][0])+abs(alist[j][1]-alist[i][1])+max(0,alist[j][2]-alist[i][2]);
        }
    }
